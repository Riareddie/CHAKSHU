...
export const ResponsiveContainer: React.FC<ResponsiveContainerProps> = ({
  ...
  return (
    <Component className={cn(
      'w-full',
      maxWidthClass,
      typeof padding === 'boolean' ? (padding ? 'px-4 sm:px-6 lg:px-8' : '') : paddingClasses,
...
export const ResponsiveStack: React.FC<ResponsiveStackProps> = ({
  ...
  const alignClasses = generateResponsiveClasses(
    align,
    (val) => {
      const alignMap = {
        start: 'items-start',
  ...
  const wrapClasses = generateResponsiveClasses(
    wrap,
...
export const ResponsiveColumn: React.FC<ResponsiveColumnProps> = ({
  ...
  const offsetClasses = generateResponsiveClasses(
    offset,
    (val) => val > 0 ? `col-start-${val + 1}` : ''
  );

  const orderClasses = order ? generateResponsiveClasses(
    order,
    (val) => `order-${val}`
  ) : '';
...
export interface ResponsiveHeadingProps {
  children: React.ReactNode;
  level: 1 | 2 | 3 | 4 | 5 | 6;
  size?: 'xs' | 'sm' | 'base' | 'lg' | 'xl' | '2xl' | '3xl' | '4xl' | ResponsiveValue<'xs' | 'sm' | 'base' | 'lg' | 'xl' | '2xl' | '3xl' | '4xl'>;
  weight?: 'normal' | 'medium' | 'semibold' | 'bold' | ResponsiveValue<'normal' | 'medium' | 'semibold' | 'bold'>;
  className?: string;
  as?: keyof JSX.IntrinsicElements;
}
